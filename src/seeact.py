...
async def run_single_task(task_id, single_query_task, config, base_dir):
    # basic settings
    ...
    main_result_path = os.path.join(save_file_dir)
            ...
            if len(elements) == 0:
                if monitor:
                    ...
                    if monitor_input in ["i", "intervene", 'intervention']:
                        ...
                        take_actions_and_elements.append({"actions": human_intervention, "elements": "none"})
                        continue
                ...
                final_json = {"confirmed_task": confirmed_task, "website": confirmed_website,
                                ...
                                "num_step": len(taken_actions), "take_actions_and_elements": take_actions_and_elements, "action_history": taken_actions, "outputs": outputs, "thoughts": thoughts,
                                "exit_by": "No elements"}
            ...
            for multichoice_i in range(0, num_choices, step_length):
                ...
                try:
                    ...
                    await session_control.active_page.screenshot(path=os.path.join(main_result_path, 'trajectory',f'{time_step}_full.jpg'), full_page=True,
                                                                    type='jpeg', quality=100, timeout=20000)
                    device_pixel_ratio = await session_control.active_page.evaluate("window.devicePixelRatio")
                ...
                if (0 <= element_id < len(candidate_ids) and pred_action.strip() in ["CLICK", "SELECT", "TYPE",
                                                                                        "PRESS ENTER", "HOVER",
                                                                                        "TERMINATE"]):
                    # thoughts.append([output0, output])
            ...
            try:
                ...
                try:
                    if target_element == []:
                        ...
                        take_action_image = cv2.imread(input_image_path)
                        bounding_box = await target_element[-2].bounding_box()
                        bounding_box_point = [bounding_box["x"], bounding_box["y"] - clip["y"], bounding_box["width"], bounding_box["height"]]
                        target_element[3] = bounding_box_point
                        # annotated_image = bounding_box_annotator.annotate(scene=take_action_image, detections=convert_elements2detections(target_element[3]))
                        # cv2.imwrite(os.path.join(main_result_path, 'trajectories',
                        #                                 f'{time_step}_{multichoice_i // step_length}_crop_grounding.jpg'), annotated_image)
                ...
                take_actions_and_elements.append({"actions": new_action, "elements": target_element[:4]})
                ...
                final_json = {"confirmed_task": confirmed_task, "website": confirmed_website,
                                "task_id": task_id, "success_or_not": success_or_not,
                                "num_step": len(taken_actions), "take_actions_and_elements": take_actions_and_elements, "action_history": taken_actions, "outputs": outputs, "thoughts": thoughts, "exit_by": str(e)}
...
if __name__ == "__main__":
    ...
    asyncio.run(run_single_task(example_task["task_id"], example_task, config, base_dir))
